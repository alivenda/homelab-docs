...
These services follow the [app deployment pattern](../deploy/index.md) exactly — workload (Helm or raw), `SealedSecret`, an `HTTPRoute` on the shared Gateway, and Authelia. Instead of a full runbook each, this page records only the **per-app deltas** that feed the pattern, plus the one or two non-obvious gotchas. The deployed truth for each is `homelab-manifests/apps/<app>/`.
...
- The **first user to sign in through OIDC becomes the server owner**; `ACTUAL_USER_CREATION_MODE=manual` (default) means the owner must add every other user in-app (matching username) before they can log in; `login` auto-provisions any Authelia user.
...
as Immich and Plex. The only thing in `apps/audiobookshelf/` is the Kubernetes glue that
...
| Kubernetes footprint | selector-less `Service` + manual `EndpointSlice` → NAS `10.0.20.50:13378`, + `HTTPRoute` (Immich-style) |
...
- **Kubernetes fronting only.** A selector-less `Service` + manual `EndpointSlice` point
  at the NAS (`10.0.20.50:13378`); the existing `HTTPRoute` for
  `audiobooks.yourdomain.com` (TLS from the Gateway wildcard cert) backs onto it.
...
- **Not ForwardAuth.** The mobile apps + browser player hit ABS's API with bearer
  ...
  scheme **`audiobookshelf://oauth`** (the one that's to miss and is what makes
...
- **Websockets**: progress sync is Socket.IO on `/socket.io/`
<!-- vale Google.Spacing = YES -->; Traefik upgrades it
...
| Secret keys | none |
| Auth | **None** — Nextcloud reaches it server-side through WOPI |
...
- **SQLite → `local-path`, not `nfs-storage`** (the old row was wrong): NFS lacks
  ...
  set by `DT_SQLITE_PATH` env (read with `os.Getenv`, **not** a YAML key) pointed
...
- **OAuth 2.0 endpoints are explicit** (authorization / token / userinfo URLs), not
...
- **Break-glass = local administrator.** `is_user_creation_disabled` closes only the
  ...
  with it `false`, register one local administrator + log in through OIDC, then flip to
  `true`.
...
- The default administrator (`changeme@example.com` / `MyPassword`) stays active until you log in through OIDC and promote your user to administrator (Settings → Users), then set `ALLOW_PASSWORD_LOGIN=false`.
...
!!! warning "Retired (2026-06)"
    ...
    [Observability](../build/observability.md) and the `infrastructure/blackbox-exporter`
