...
!!! note "Decision record, not the status page"
    ...
    current status see the [App Catalog](app-catalog.md) and the
...
| **DNS node (pyrite — Pi 3 Model B)** | ARM64, 1 GB RAM | AdGuard Home (optional second, marcasite, for failover) |
...
| 🌐 **DNS** | Run on a dedicated Raspberry Pi (pyrite; optional second) |
...
**Category:** DNS | **Run:** 🌐 Raspberry Pi — pyrite (optional second, marcasite, for failover) | **RAM:** ~100 MB | **ARM64:** ✅ Official multiarch | **Runbook:** [AdGuard Home](../deploy/adguard-home.md)
...
**Category:** Federated Identity | **Run:** 🖥️ Cluster | **RAM:** ~100–150 MB | **ARM64:** ✅ Official | **Runbook:** [Authelia](../deploy/authelia.md)
...
- **OIDC** for apps with their own login (Nextcloud, Paperless, Forgejo, Miniflux, …) — they redirect to Authelia and manage their own session. **Do not** put forward-auth in front of these: their API clients (desktop sync, git command-line tool, mobile apps) send credentials directly and can't follow the redirect.
- **ForwardAuth** (Traefik middleware) only for apps with *no* login of their own (for example, Homepage).
- 2FA (TOTP, WebAuthn) and single sign-on across all of the preceding in one place.
- LDAP-compatible user store through **lldap** (Rust, ~30 MB, ARM64 ✅ — lightweight LDAP backend), or flat file for simplicity.

**Why before everything else:** every new service you add needs auth. Doing it now means each future service gets SSO for free.
...
**Category:** Communication | **Run:** 🖥️ Cluster | **RAM:** ~50 MB | **ARM64:** ✅ Official | **Runbook:** [ntfy](../deploy/ntfy.md)
...
<!-- vale Homelab.Headings = YES -->
**Category:** Feed Readers | **Run:** 🖥️ Cluster | **RAM:** ~50 MB | **ARM64:** ✅ Official | **Runbook:** [Miniflux](../deploy/miniflux.md)

**Miniflux** — Go binary, minimal, fast, Fever + Google Reader API (works with Reeder, NetNewsWire, etc.), very low RAM. PostgreSQL-only, so it rides the shared [NAS Postgres](../deploy/nas-postgres.md) and runs fully stateless in-cluster.
...
### ⬛ Bookmarks — linkding *(retired here)*
**Category:** Bookmarks | **Run:** 🖥️ Cluster | **RAM:** ~100–150 MB | **ARM64:** ✅ Official | **Runbook:** [App Catalog](app-catalog.md#linkding)

Retired from this build (2026-09): it went unused, because Firefox's own bookmarks already cover the need. Still a fine pick if you want bookmarks outside the browser.
...
### ⬛ Tasks / to-do — Vikunja *(retired here)*
**Category:** Task Management | **Run:** 🖥️ Cluster | **RAM:** ~150–200 MB | **ARM64:** ✅ Official | **Runbook:** [Vikunja](../deploy/vikunja.md)

Retired from this build (2026-09): it went unused because no task or kanban workflow needed it. Still a fine pick if you want a self-hosted task manager.
...
**Category:** Media Management | **Run:** 💾 NAS or 🖥️ Cluster (pointing at NAS NFS storage) | **ARM64:** ✅ All official | **Runbook:** [Arr Stack](../deploy/arr-stack.md)
...
**Category:** Document Mgmt / Audio | **Run:** 💾 NAS (Docker) | **RAM:** ~200–400 MB | **ARM64:** ✅ Official | **Runbook:** [App Catalog](app-catalog.md#audiobookshelf)
...
**Category:** Personal Dashboards | **Run:** 🖥️ Cluster | **RAM:** ~150–200 MB | **ARM64:** ✅ Official | **Runbook:** [Homepage](../deploy/homepage.md)
...
## Part 4 — Good additions (after the preceding are stable)
...
### 🟡 Wiki — BookStack
**Category:** Wikis | **Run:** 🖥️ Cluster | **RAM:** ~200–300 MB | **ARM64:** ✅ Official | **Runbook:** [BookStack](../deploy/bookstack.md)
...
**Category:** Monitoring | **Run:** 🖥️ Cluster | **RAM:** ~150–200 MB | **ARM64:** ✅ Official | **Runbook:** [App Catalog](app-catalog.md#uptime-kuma)
...
**Category:** Miscellaneous / Task Management | **Run:** 🖥️ Cluster | **RAM:** ~100 MB | **ARM64:** ✅ (v0.1.75 multi-arch) | **Runbook:** [App Catalog](app-catalog.md#donetick)
...
**Category:** Miscellaneous | **Run:** 🖥️ Cluster | **RAM:** ~300–500 MB (app + PostgreSQL + Redis) | **ARM64:** ✅ Official | **Runbook:** [Reactive Resume](../deploy/reactive-resume.md)
...
!!! tip "Consider the desktop instead"
    Your daily driver desktop (RTX 3080) runs 7–13B models an order of
...
| **Bookmarks** | ⬛ linkding (retired) | See Part 3 |
...
| **File Transfer — Sync** | ✅ Nextcloud + 🟡 Syncthing | Syncthing (Go, ARM64 ✅) for P2P sync without server-round-trip — [Syncthing](../deploy/syncthing.md) |
| **Games** | ⚪ Skip | |
| **Games — Administrator Panels** | ⚪ Skip | No game servers in this setup |
| **Genealogy** | ⚪ Skip | |
...
| **Media Management** | ⬛ Arr stack — shelved, never deployed | See Part 3; [Arr Stack](../deploy/arr-stack.md) has the full reasoning |
...
| **Remote Access** | ✅ Tailscale + 🟡 RustDesk for graphical remote control | RustDesk server: Rust, ~50–100 MB, ARM64 ✅ — [RustDesk](../deploy/rustdesk.md); Tailscale handles network access, RustDesk handles the screen |
...
| **Task Management** | ⬛ Vikunja (retired) | See Part 3 |
...
<!-- vale Vale.Terms = NO -->
Self-hosting your own mail server (Mailcow, docker-mailserver, Stalwart)
<!-- vale Vale.Terms = YES --> is the most common homelab trap:
...
If it's for yourself: Signal is fine. Don't deploy a chat server unless you have people who'll use it.
...
7.  linkding                  (retired)        → removed 2026-09, unused
8.  Actual Budget             (catalog)        → financial tracking
9.  Vikunja                   (retired)        → removed 2026-09, unused
...
- Arr stack on NAS, if ever revived (direct disk access, no NFS hop for downloads; at 8 GB it is tight during Plex transcode + Immich ML spikes, so cluster+NFS was the planned fallback)
...
| ⬛ Retired | linkding | Browser cloud bookmarks — removed 2026-09, see Part 3 |
| ⬛ Retired | Vikunja | Todoist, TickTick — removed 2026-09, see Part 3 |
| 🔴 High | Actual Budget | YNAB |
| ⬛ Shelved | Arr stack | Manual Plex library management — decided against, see [Arr Stack](../deploy/arr-stack.md) |
