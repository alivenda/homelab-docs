...
ntfy follows the [deploy pattern](index.md) in **raw-manifests mode**: there is no first-party Helm chart, only third-party repackages — an avoidable dependency for a handful of small files (same call as Homepage). That also means **one** ArgoCD `Application` pointing at `apps/ntfy/manifests/`; with no chart source there is nothing to split a second `-manifests` Application from.
...
Both persistent stores are **SQLite**, so both PVCs bind the node-local `local-path` class — *not* `nfs-storage`, which corrupts SQLite (see [Storage & Data Architecture](../concepts/storage.md)):
...
1. **Publish the hostname** — add `ntfy` to the service list in the Cloudflare [Terraform](../build/terraform.md) module and apply: one A record per service pointing at the Traefik LB IP. The route can't be reached by name until this exists.
2. **HTTPRoute** per the [deploy pattern](index.md) — plain, on the shared Gateway's `websecure` listener; the wildcard cert terminates at the Gateway.
...
- [ ] **Backup captured real bytes** — run an on-demand velero backup of the namespace and check `PodVolumeBackup` progress for **non-zero** `totalBytes` on *both* volumes. `Completed` alone proves nothing (see [Backups & DR](../build/backups.md)).
