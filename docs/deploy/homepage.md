...
Homepage follows the [deploy pattern](index.md) in **raw-manifests mode**: there is no first-party Helm chart — upstream's own docs link an unofficial one, the same avoidable third-party dependency ntfy rejects. One ArgoCD `Application` pointing at `apps/homepage/manifests/`:
