...
[SQLite-on-NFS corrupts](../concepts/storage.md). Vikunja speaks PostgreSQL natively,
so the [decomposition rule](../concepts/storage.md#how-to-decompose-one-app) applies
...
## The config-surface gotchas { #step-2-the-config-surface-gotchas }

Two things the env-var convention hides:

1. **OIDC providers cannot be configured purely through env.** Vikunja only reads
   ...
   arrives through env from the SealedSecret. The mount is `subPath`, so config changes
   need a pod restart.
...
reuse the DB password from [Database on the NAS](#step-1-database-on-the-nas), and seal all three (exact command in the app
