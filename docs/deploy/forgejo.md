...
!!! warning "This replaces the old single-NFS-PVC approach"
    Earlier revisions of this runbook put the whole `/data` tree on `nfs-storage`. That silently risks SQLite corruption. Always split: **DB on `local-path`, repos on `nfs-storage`.** See [Storage & Data Architecture](../concepts/storage.md).
