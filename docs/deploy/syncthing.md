...
Deploy through the NAS Docker interface or `docker compose up -d`.

### Other devices (Linux servers, Raspberry Pi)

Install with the official script or package manager, then enable the service:
...
## Pair devices { #step-2-pair-devices }
...
## Share folders { #step-3-share-folders }
...
3. In the **Sharing** tab, tick the remote devices that receive this folder.
...
## Secure the web UI { #step-4-secure-the-web-ui }
