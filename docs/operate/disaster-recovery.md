...
| **Step zero, always** | [Secrets and key-material recovery](../build/backups.md#secrets-and-key-material-recovery) — nothing restores until you can decrypt |
| **Planned downtime instead?** | A move or long storage is not a disaster — use [Cold Shutdown](cold-shutdown.md) |
| **Last drilled** | Secrets chain (scenario 1 + the Garage path): 2026-07-17, [full pass](../build/backups.md#drill-the-whole-chain-no-dead-machine-required) |
...
2. Reflash DietPi on the module — [Turing Pi, Step 3](../build/turing-pi.md#step-3-flash-dietpi-os-to-all-4-modules).
3. Re-run `site.yml` from [Ansible](../build/ansible.md) — it reinstalls the **pinned** k3s agent, rejoins with the same token, and re-applies the node's labels from `inventory.yml` (including `app-state=true`). Confirm: node `Ready`, labels present in `kubectl get nodes --show-labels`.
...
5. Verify content, not exit codes: open each restored app and check real data, per [Test your restores](../build/backups.md#test-your-restores). Finish with ArgoCD all `Synced`/`Healthy`.
...
1. Reflash the module — [Turing Pi, Step 3](../build/turing-pi.md#step-3-flash-dietpi-os-to-all-4-modules) — and reconnect the SSD ([Step 5](../build/turing-pi.md#step-5-prepare-sata-ssd-on-node-3-topaz-node-3) covers the physical side; don't run its format commands on a disk with data).
...
The data plane keeps serving — apps stay up, you just can't deploy, scale, or restart anything. Don't power-cycle the agents while ruby is down; their workloads are fine. The full procedure with commands is [Kubernetes, Step 11](../build/kubernetes.md#step-11-restore-procedure-when-ruby-dies); the order:

1. Reflash the module — [Turing Pi, Step 3](../build/turing-pi.md#step-3-flash-dietpi-os-to-all-4-modules).
2. Re-run `site.yml` from [Ansible](../build/ansible.md) — pinned k3s server install plus the `etcd-s3` config the restore reads.
3. Restore etcd from the newest Garage snapshot with `--cluster-reset` — [Step 11](../build/kubernetes.md#step-11-restore-procedure-when-ruby-dies) has the exact commands.
...
2. Give it back its static IP (`10.0.20.50` — [static-IP table](../build/network.md#static-ip-allocations)).
3. Start the Docker stacks: Garage ([stand up Garage](../build/backups.md#stand-up-garage) is the reference), Postgres ([NAS PostgreSQL](../deploy/nas-postgres.md#stand-up-the-server)), [Immich](../deploy/immich.md), [Audiobookshelf](../reference/app-catalog.md#audiobookshelf).
4. A UGOS reinstall resets host-level systemd units — recreate the sync timers from [the rclone → Garage pattern](../build/backups.md#nas-side-sync-jobs-the-rclone-garage-pattern) and confirm every timer in [What runs when](../build/backups.md#what-runs-when) shows a next run.
...
!!! danger "Accepted data loss until off-site backup exists"
    Everything whose only copies lived on the NAS is gone: the entire Garage backup history, the shared Postgres databases **and** their dumps, and every Immich original not still on a device. This is exactly the gap the [off-site TODO](../build/backups.md#what-runs-when) exists to close.

1. Rebuild the NAS: fresh UGOS, static IP, then [stand up Garage](../build/backups.md#stand-up-garage) and re-run the bucket/key/allow trio for every consumer.
...
3. Stand the Postgres tier back up and re-provision each app's database and role ([NAS PostgreSQL](../deploy/nas-postgres.md#provisioning-a-database-for-an-app)); apps recreate their schemas on first start, with empty tables.
4. Redeploy [Immich](../deploy/immich.md) and [Audiobookshelf](../reference/app-catalog.md#audiobookshelf); re-upload photos from devices; let Audiobookshelf re-scan the (also lost) media you restore or re-rip.
...
- Force fresh backups immediately instead of waiting for the schedules, and confirm them with the [Backups verification list](../build/backups.md#verification).
