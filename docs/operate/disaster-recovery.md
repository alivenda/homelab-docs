...
The data plane keeps serving — apps stay up, but you can't deploy, scale, or restart anything. Don't power-cycle the agents while ruby is down; their workloads are fine. The full procedure with commands is [Restore procedure (when ruby dies)](../build/kubernetes.md#step-11-restore-procedure-when-ruby-dies); the order:

1. Reflash the module — [Flash DietPi](../build/turing-pi.md#step-3-flash-dietpi-os-to-all-4-modules).
2. Re-run `site.yml` from [Ansible](../build/ansible.md) — pinned k3s server install plus the `etcd-s3` config the restore reads.
3. Restore etcd from the newest Garage snapshot with `--cluster-reset` — [Restore procedure](../build/kubernetes.md#step-11-restore-procedure-when-ruby-dies) has the exact commands.
4. Agents rejoin on their own (same token, same server IP). Run the post-restore checks from that section: nodes `Ready`, sealed-secrets signing keys present, ArgoCD green.
