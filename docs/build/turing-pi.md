...
Initial hardware assembly, firmware update, OS flashing, and network configuration for your Turing Pi 2 with 4x Raspberry Pi CM4 modules (mixed eMMC capacity).
...
| **Networking** | Onboard 1 Gbps managed switch, 2x RJ45 (bridged) |
| **Storage I/O** | 2x SATA III (Node 3), 4x M.2 slots (NOT usable with CM4) |
...
## Hardware assembly { #step-1-hardware-assembly }
...
## Flash DietPi OS to all 4 modules { #step-3-flash-dietpi-os-to-all-4-modules }
...
Node 3 connects to the SATA ports. This SSD serves as NFS storage for the entire cluster.

1. SSH into topaz as `dietpi` and identify the disk with `lsblk` (don't assume `/dev/sda`)
2. Format and mount:
...
!!! tip "Reserve DHCP ranges"
    Reserve IPs 10.0.20.10–13 for your nodes and 10.0.20.200–250 for MetalLB in your UDM DHCP settings (see [Network](network.md) for the full Lab VLAN plan).

## Power: UPS + NUT for graceful shutdown { #power-ups-nut-for-graceful-shutdown }
...
!!! tip "Test the shutdown cascade"
    Test the shutdown flow by unplugging the UPS from the wall (not the equipment from the UPS). You want to see the cascade happen before a real outage proves your config wrong.
