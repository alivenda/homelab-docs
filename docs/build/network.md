...
This build runs **AdGuard Home** on a dedicated Pi (`pyrite`, `10.0.0.20` on the Default LAN — see [AdGuard Home](../deploy/adguard-home.md)). This step does two firewall jobs around it: let every VLAN *reach* the resolver, then force *all* client DNS through it.
