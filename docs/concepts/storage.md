...
storage choices the runbooks and the [App Catalog](../reference/app-catalog.md) assume — read it
...
running many small app pods, weak at database I/O. Data is split by **what it is**,
...
runs Immich's Postgres, extending an established pattern rather than inventing one.
...
- **Access:** `pg_hba` allows each role only its own database, only from the four node IPs
  (pod egress is SNAT'd to node IPs); no superuser over TCP. The server stays on the Lab
  VLAN, never exposed externally.

[NAS PostgreSQL](../deploy/nas-postgres.md) is the implementation.
...
Revisit CloudNativePG only if the nodes ever gain per-node NVMe.
...
— see the [danger note in the local-path tier section](#the-local-path-tier). `hostPath` volumes are silently skipped.
...
- **BookStack and Reactive Resume assume cluster-hosted databases** — revise their DB sections to
  ...
  was trued up at its 2026-06 bring-up.)
